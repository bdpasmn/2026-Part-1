...
$message = '';
$attempts_left = 3; // default before any POST
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($first) || empty($last) || empty($email) || empty($password)) {
        ...
        $statement = $pdo->prepare('
            SELECT password, user_id, title, role, email, first_name,
        ...
        if (!$row) {
            ...
            $timeSinceFail = $row['last_failed_at']
                ? time() - strtotime($row['last_failed_at'])
                : 3601;

            // Pull attempts_left directly from DB
            $attempts_left = $timeSinceFail >= 3600
                ? 3
                : max(0, 3 - $row['failed_attempts']);
            ...
            if ($row['failed_attempts'] >= 3 && $timeSinceFail < 3600) {
                ...
                $message = $attempts_left <= 0
                    ...
                    : "Invalid credentials. $attempts_left attempt(s) left.";
...
<main class="flex flex-grow items-center justify-center bg-gradient-to-r from-slate-900 to-slate-800 p-6">
    ...
    <div class="w-full max-w-3xl space-y-6">
        ...
        <div class="text-center bg-gray-800 border border-gray-700 rounded-xl p-8 shadow-lg">
            ...
            <a
                class="mb-3 text-red-100 hover:text-red-300"
                href="<?= BASE_URL ?>/pages/accountRecovery/recovery.php">
        ...
        <div class="text-center bg-gray-800 border border-gray-700 rounded-xl p-8 shadow-lg">
            ...
            <form method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    <label class="flex justify-center text-xs text-gray-400">
                        Remember Me?
                    </label>

                    <input
                        class="mt-1 h-10 bg-gray-700 border border-gray-600 rounded-lg px-3 scale-150"
                        type="checkbox"
                        name="remember"
                        value="yes">
                </div>
                ...
                <div class="text-center md:col-span-2 mt-4">
                    <input
                        class="bg-blue-600 text-white px-6 py-2 rounded transition duration-200 hover:bg-blue-700 hover:shadow-md active:scale-95"
                        type="submit"
                        value="Login">
                </div>
