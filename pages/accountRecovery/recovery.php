                ...
                <div class="max-w-3xl mx-auto bg-gray-800 border border-gray-700 rounded-xl p-8 shadow-lg">
                    ...
                    <form method="POST" class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="email" value="<?= htmlspecialchars($email) ?>" <?= !empty($userSecurity) ? "readonly class='w-full h-12 px-4 bg-gray-600 border border-gray-500 rounded-lg text-gray-300 cursor-not-allowed'" : "class='w-full h-12 px-4 bg-gray-700 border border-gray-600 rounded-lg text-white transition duration-200 focus:outline-none focus:ring-1 focus:ring-white focus:border-white'" ?> required placeholder="Enter your email">
                        </div>

                        <?php if (!empty($userSecurity)): ?>
                            <a href="?reset=1" class="inline-flex items-center gap-2 mb-4 px-4 py-2 text-sm font-medium text-gray-200 bg-gray-700 border border-gray-600 rounded-lg hover:bg-gray-600 hover:border-gray-500 hover:-translate-y-0.5  transition duration-200 shadow-sm">← Change Email</a>
                        ...
                        <?php else: ?>
                            <div class="text-gray-400">Enter your email to load your security questions.</div>
                        <?php endif; ?>

                        <button type="submit" class="w-full h-12 bg-blue-600 text-white rounded-lg font-medium transition duration-200 hover:bg-blue-700 hover:shadow-md">Recover Account</button>
                    </form>
                </div>
            </section>
        </div>
